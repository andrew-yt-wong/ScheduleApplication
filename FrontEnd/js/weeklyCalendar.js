			...
			$.each(dataArr,function(ind,ele) {
				...
				if (ele_year == show_date[0] && ele_month == show_date[1] && ele_date == show_date[2]) {
					...
					$.each(ele.items,function(_index,_ele){
						optionsHtml += '<li style="background-color: '+_ele.color+'">'
									+ '<p>'+_ele.classTime+'</p>'
									+ '<p>'+_ele.className+'</p>'
							if (showData) {
							optionsHtml	+= '<article>'
								+ '<a href="javascript:void(0)" onclick="editInf(this)">'+description.firstTitle+'</a>'
								+ '<a href="javascript:void(0)" onclick="deleteInf(this)">'+description.nextTitle+'</a>'
								+ '</article>'
